# 开发历史归档（原 STATUS.md 全文，2026-07-21 归档）

> 本文件是只增不改的历史日志。当前权威状态见根目录 `STATUS.md`；定位见 `docs/POSITIONING.md`。

# STATUS

## SIM LAB 与 REAL 串口会话彻底分屏（2026-07-18）

- 成品目视审查发现：默认 REAL 工作区已经没有仿真设备，但显式进入 SIM LAB 后，右侧证据栏底部仍挂着一条可展开的 `REAL HARDWARE` 控制条。这会重新制造“左边虚拟、右边真实并列操作”的错误产品心智。
- SIM LAB 现在不挂载也不显示任何真机控制；切入时由 REAL 面板卸载清理逻辑断开串口、停止轮询并清空距离和最后指令角度。连接状态下的模式按钮会在动作前明确显示“断开 → SIM”，实验室说明也持续声明无真机控制、进入即断开和清空遥测。
- REAL 控制、示教、固件和回显只存在于默认 REAL 工作区；仿真实验室仅保留指令理解、动作/规则审查与零信号审计用途，不能与真实执行同屏或充当失败回退。
...
- 现统一为 REAL-device-first：断开页直接定义当前唯一已审真机台架 `ESP32-S3 + SG90 + HC-SR04`，显示 SG90 GPIO18、HC-SR04 TRIG GPIO5、ECHO GPIO4 及强制 5V→3.3V 分压，并明确其他板卡/引脚/传感器不在当前可执行范围；离线指南补齐自动检测、空白板首次烧录、真实执行边界和开环位置诚实语义。
- SIM LAB 在帮助中只被定义为提案解析、规则、动作和审计呈现的次级零信号测试工具；不证明真实环境安全，也不替代失败的 REAL 操作。desktop/conformance 新增对实际渲染 HTML 与 About 源的回归断言，禁止旧定位再次出现。

## 空白 ESP32-S3 的内置首次烧录闭环（2026-07-18）

- 产品根因：此前“零 IDE”只对已经运行 RealityWarden 固件、能完成握手的设备成立；真正的空白板无法先连接，因此也无法取得固件预览，形成首次使用死锁。现在自动检测仍先做只读探测；若端口已被操作系统枚举但固件空白或无响应，REAL 工作区会把该端口交给显式的“准备首次烧录”，不伪造连接或诊断成功。
- 权威边界：主进程只解析仓库内已审镜像或合法写入令，随后把当前枚举端口指纹、规范化请求摘要、镜像 SHA-256 与随机授权 ID 绑定。授权最多五分钟、一次尝试即消费；烧录前重新枚举并逐项核对，端口消失/换板、请求变化、镜像变化或到期一律拒绝。并发锁在首次异步检查前取得；芯片识别和写入仍由现有 esptool-js 路径完成，非 ESP32-S3 在写入前拒绝，无任意 BIN、任意路径、现场编译或静默重试。
- 诚实恢复：烧录前若失败明确报告 `not_written`；进入写入后失败报告硬件状态不确定。成功后才重连并运行 diagnose 核对版本。固件维护不新增 actuation IPC，舵机执行仍只走 HardwareExecutionGate ticket 通路；SIM LAB 仍是显式、次级、零信号工具。
- 验证：root/Next/Electron typecheck、device-onboarding、real-hardware 48/48、virtual-loopback 5/5、desktop、conformance、accessibility 与完整 `npm run verify` 全绿；REAL/SIM 产品设计矩阵在 1440x900 与 1180x720、中英文及 Windows 125%/150% 下零违规。

## REAL 主任务栏与连接引导收口（2026-07-18）✅

- 产品实测发现断开页此前宣传“选择端口 → 诊断 → 显式连接”，但界面并不存在独立 Diagnose 控件；权威实际路径是“插入参考设备 → 自动检测（只读扫描 + diagnose）→ 自动连接”。断开引导、主按钮和中英文说明现与真实行为一致，并明确自动检测不会驱动舵机；手动刷新/选端口/连接保留为次级路径。
- REAL 模式右栏不再用重复的“设备状态 / 安全契约”Tab 与连接、指令、示教、固件争抢高度。独立 `REAL HARDWARE` 治理边界现占满右侧任务栏；连接状态、遥测、执行证据和安全解释仍在边界、顶栏、只读孪生体与逐次结果中按上下文呈现，六条不变量及执行路径零改动。
- SIM LAB 中的 REAL HARDWARE 边界默认折叠，仅保留明确警示入口；回到 REAL 模式仍由 `primary` 强制展开。Electron 设计验收从“真机边界至少占右栏一半”收紧为“至少 95%”，desktop/conformance/launch-closure 契约同步防回归。
