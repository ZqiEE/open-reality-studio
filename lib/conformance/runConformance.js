...
function main() {
  ...
  assert(realHardwarePanel.includes('Reference-servo preflight (no signal)') && realHardwarePanel.includes('This is not the generic simulation workspace on the left.'), 'REAL preflight must remain visibly local to the reference rig and must not imply a generic simulation-to-real mapping.');
  ...
  assert(realDeviceWorkspace.includes('REAL DEVICE WORKSPACE') && realDeviceWorkspace.includes('workspaceDevices={[]}') && realDeviceWorkspace.includes('No virtual device is placed here'), 'The REAL-first workspace must not place a virtual model or imply a simulation-to-real mapping.');
