...
function main() {
  ...
  const realDeviceWorkspace = fs.readFileSync(path.join(root, 'components/RealDeviceWorkspace.tsx'), 'utf8');
  ...
  const mainProcess = fs.readFileSync(path.join(root, 'electron/main.ts'), 'utf8');
  const hardwareIpc = fs.readFileSync(path.join(root, 'electron/ipc/hardware.ipc.ts'), 'utf8');
  const supportGuideHtml = fs.readFileSync(path.join(root, 'docs/SUPPORT.html'), 'utf8');
  const supportActions = fs.readFileSync(path.join(root, 'electron/support/supportActions.ts'), 'utf8');
  const firmwareTargetAuthorization = fs.readFileSync(path.join(root, 'lib/device-onboarding/FirmwareTargetAuthorization.ts'), 'utf8');
  const realExecutionReleaseApproval = fs.readFileSync(path.join(root, 'lib/hardware/RealExecutionReleaseApproval.ts'), 'utf8');
  ...
  assert(realHardwarePanel.includes('Reference-servo preflight (no signal)') && realHardwarePanel.includes('it has no mapping to SIM LAB'), 'REAL preflight must remain visibly local to the reference rig and deny any Simulation Lab mapping.');
  ...
  assert(virtualDeviceStage.includes('SIMULATION · REAL MIRROR SEPARATE') && virtualDeviceStage.includes('SIMULATION · NO HARDWARE SIGNAL') && virtualDeviceStage.includes('READ-ONLY DIGITAL TWIN'), 'The workspace must keep simulation, no-signal, and separate connected REAL mirror semantics simultaneously visible.');
  assert(page.includes('[SIMULATION]') && page.includes('no signals were sent to real hardware'), 'Invariant 6 (simulation and reality are visibly distinct): simulated runs must be marked [SIMULATION] with an explicit no-hardware-signal statement, so a user can always tell which world they are acting in.');
  ...
  assert(readme.includes('The app opens in the REAL workspace') && readme.includes('Without hardware, explicitly enter **SIM LAB**'), 'README must lead with the REAL product path and present no-hardware simulation as an explicit secondary choice.');
  assert(
    readme.includes('simulation is explicit and never a fallback from REAL'),
    'README must keep simulation explicit and forbid fallback from REAL.'
  );
  assert(readme.indexOf('default desktop shell is REAL-device-first') < readme.indexOf('SIM LAB remains'), 'README must position REAL before the secondary Simulation Lab.');
  assert(page.includes('<RealDeviceWorkspace') && page.includes('<RealHardwarePanel'), 'The REAL-first shell must keep visualization separate from the existing governed hardware panel.');
  assert(!evidenceSidebar.includes('data-real-hardware-primary="false"') && (evidenceSidebar.match(/{hardware}/g) || []).length === 1, 'REAL HARDWARE controls must mount only in REAL mode and be absent from Simulation Lab.');
  assert(appHeader.includes("'DISCONNECT → DRY RUN'") && labConfigurator.includes('Simulation Lab has no hardware controls') && labConfigurator.includes('disconnects any REAL session and clears REAL telemetry'), 'Entering Simulation Lab from a connected REAL session must visibly disclose the disconnect and clear separation.');
  assert(mainProcess.includes('!snapshot.realHardwareBoundary && !snapshot.realHardwarePrimary && !snapshot.realHardwarePanelReady') && mainProcess.includes("if (hardware) violations.push('real_hardware_visible_in_simulation')"), 'Packaged smoke and design acceptance must reject any REAL hardware surface inside Simulation Lab.');
  ...
  assert(evidenceSidebar.includes("if (workspaceMode === 'real')") && evidenceSidebar.indexOf('{hardware}') < evidenceSidebar.indexOf('role="tablist"') && (evidenceSidebar.match(/{hardware}/g) || []).length === 1, 'REAL HARDWARE must occupy the REAL-only task rail and never appear in Simulation Lab.');
  assert(evidenceSidebar.includes('data-real-task-sidebar') && evidenceSidebar.includes('data-real-hardware-primary="true"') && !page.includes('Real-device safety contract'), 'REAL mode must not sacrifice hardware task space to duplicate status or developer-contract panels.');
  ...
  assert(page.includes('function deriveRunState(') && page.includes('const runState = useMemo(() => deriveRunState({'), 'Run state must have one authoritative source; no surface may derive its own run phase.');
  assert(page.includes('result={runState.headerResult}') && page.includes('status={runState.command}') && page.includes('running={runState.active}') && page.includes('evidenceKey={runState.evidenceKey}'), 'The header, AI terminal, and evidence rail must all subscribe to the single run state.');
  assert(!page.includes("labReport?.result === 'blocked' ? 'blocked' : replayPlaying ? 'running'"), 'The header must not re-derive a second run phase from the lab report.');
  assert((page.match(/data-run-control/g) || []).length === 1 && (page.match(/data-stop-control/g) || []).length === 1 && commandDockSource.includes('data-run-control') && commandDockSource.includes('data-stop-control'), 'Exactly one Run control and one Stop control may exist, and both must live in the AI terminal (CommandDock).');
  assert(!appHeader.includes('data-run-control') && !appHeader.includes('data-stop-control') && !appHeader.includes('onRun') && !appHeader.includes('onStop'), 'The top bar must carry no Run/Stop control that duplicates the AI terminal.');
  assert(page.includes('function RefusalCard(') && page.includes("runState.phase === 'blocked' && (") && page.includes('<RefusalCard'), 'A refusal must render its own dedicated card off the single run state, not just a status word.');
  assert(page.includes("'拦了什么' : 'What was refused'") && page.includes("'为什么' : 'Why'") && page.includes("'下一步' : 'What next'"), 'The refusal card must always answer what was refused, why, and what to do next — a blocked user may never hit a dead end.');
  assert(page.includes('data-refusal-export-receipt') && page.includes('onExportReceipt={() => void exportCurrentAuditReceipt()}'), 'The refusal card must offer the audit receipt directly: refusing is the product, and the receipt is the deliverable.');
  assert(page.includes("const noticeSeverity = 'warning';") && !page.includes('[ERROR] Blocked before adapter dispatch'), 'A refusal must not be voiced as an error: the governed-decision notice severity is unconditionally non-error and no [ERROR] console prefix may describe a block.');
  assert(page.includes('[GATE] Refused before adapter dispatch'), 'A refusal blocked before adapter dispatch must be logged as a governed gate decision.');
  assert(page.includes("useState<'real' | 'simulation'>('real')") && page.includes("workspaceMode === 'simulation' && <LabConfigurator"), 'The primary shell must be REAL-first; the simulation navigator may render only after explicit mode selection.');
  assert(realDeviceWorkspace.includes("telemetry.connected ? (") && realDeviceWorkspace.includes('data-real-twin-stage') && realDeviceWorkspace.includes('data-real-device-onboarding') && realDeviceWorkspace.includes('NO 3D STAGE · NO STALE DATA'), 'The disconnected REAL-first workspace must contain no 3D stage; only a current connection may mount the read-only REAL twin.');
  assert(realDeviceWorkspace.includes("'点击自动检测'") && realDeviceWorkspace.includes('Auto-detect is read-only and never moves the servo.') && !realDeviceWorkspace.includes("'Diagnose'"), 'REAL onboarding must lead with the actual read-only auto-detect flow and never advertise a nonexistent Diagnose control.');
  assert(realDeviceWorkspace.includes('a blank or unresponsive board enters governed first flash') && realHardwarePanel.includes('if (result.results[0]?.path) setSelectedPort(result.results[0].path);'), 'Auto-detect must route an enumerated blank board to first flash rather than strand the user after handshake failure.');
  assert(realDeviceWorkspace.includes('data-supported-real-rig') && realDeviceWorkspace.includes('ESP32-S3 + SG90 + HC-SR04') && realDeviceWorkspace.includes('5V→3.3V divider required'), 'REAL onboarding must define the exact reviewed hardware and safety-critical ECHO divider in product context.');
  assert(supportGuideHtml.includes('REAL-device first') && supportGuideHtml.includes('SIM LAB is an explicit, secondary, zero-signal tool') && !supportGuideHtml.includes('simulation first') && supportActions.includes('REAL-device-first safety governance runtime.') && !supportActions.includes('Simulation-first safety governance desktop.'), 'Offline support and About must preserve REAL-first product truth and demote SIM LAB to its zero-signal review purpose.');
  assert(realHardwarePanel.includes('data-first-flash-entry') && realHardwarePanel.includes('No handshake required') && hardwareIpc.includes('consumeFirmwareAuthorization(payload.authorizationId)') && hardwareIpc.includes('delete(authorizationId); // one use'), 'A blank listed ESP32-S3 must have an honest first-flash path guarded by a one-use main-process authorization.');
  assert(firmwareTargetAuthorization.includes('target_port_not_listed') && firmwareTargetAuthorization.includes('firmware_target_changed') && firmwareTargetAuthorization.includes('firmware_request_changed') && firmwareTargetAuthorization.includes('firmware_image_changed'), 'First flash must fail closed on arbitrary paths, port swaps, request changes, or image changes.');
  assert(hardwareIpc.includes("'hardware', 'RealExecutionReleaseApproval'") && !hardwareIpc.includes('readdirSync(EVIDENCE_DIR)') && hardwareIpc.includes("!app.isPackaged && process.env.ORS_REAL_EXECUTION === 'enabled'"), 'Packaged REAL execution must use the compiled release authority, never arbitrary file counts or the source bench override.');
  assert(realExecutionReleaseApproval.includes("profileId: 'esp32_s3_sg90_hc_sr04_v1'") && (realExecutionReleaseApproval.match(/sha256: '[a-f0-9]{64}'/g) || []).length === 4 && realExecutionReleaseApproval.includes('approved_evidence_semantics_mismatch'), 'REAL release approval must bind the exact profile and all four reviewed scenario digests and semantics.');
  assert(labConfigurator.includes('data-component="DeviceNavigator"') && labConfigurator.includes('data-component="AssetLibrary"'), 'Explicit Simulation Lab mode must retain separate device configuration and asset-library regions.');
  ...
  assert(labConfigurator.includes("'Safe Rehearsal'") && labConfigurator.includes("'Simulation only · no hardware signal'") && appHeader.includes('Dry run'), 'Secondary Simulation Lab mode must remain explicitly no-signal and visibly separate from the default REAL workspace.');
  ...
  assert(appHeader.includes("const realMode = props.workspaceMode === 'real'") && appHeader.includes('data-real-hardware-focus') && appHeader.includes('data-simulation-toolbar-action="quick-start"') && appHeader.includes('data-simulation-toolbar-action="export-adapter"'), 'Header actions must be contextual: REAL device control is primary, while simulation onboarding/actions/exports appear only in Simulation Lab.');
  assert(appHeader.includes("workspaceMode === 'simulation' ? [") && appHeader.includes('data-simulation-file-action=') && appHeader.includes('Import Simulation Asset'), 'Simulation imports must be absent from the REAL File menu and explicitly labeled inside Simulation Lab.');
  assert(page.includes("if (action === 'run:virtualLab') {") && page.includes("setWorkspaceMode('simulation');") && page.includes("if (action === 'run:replay') {"), 'Desktop simulation menu actions must switch to the visibly distinct Simulation Lab before running.');
  ...
  console.log('- Main UI defaults to an honest REAL-device workspace; Simulation Lab is explicit and real execution stays inside the evidence-locked REAL HARDWARE boundary.');
  console.log('- One run has one authoritative state: header, AI terminal, and evidence rail subscribe to it, and Run/Stop exist exactly once.');
  console.log('- A refusal is a showable outcome, not an error: it states what/why/what-next and offers the audit receipt on the spot.');
  console.log('- README leads with the REAL product path and keeps Simulation Lab explicit, secondary, and non-fallback.');
