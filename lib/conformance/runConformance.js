...
function main() {
  ...
  assert(appHeader.includes("'DISCONNECT → SIM'") && labConfigurator.includes('Simulation Lab has no hardware controls') && labConfigurator.includes('disconnects any REAL session and clears REAL telemetry'), 'Entering Simulation Lab from a connected REAL session must visibly disclose the disconnect and clear separation.');
  ...
  assert(labConfigurator.includes("'Safe Rehearsal'") && labConfigurator.includes("'Simulation only · no hardware signal'") && appHeader.includes('Simulation Lab'), 'Secondary Simulation Lab mode must remain explicitly no-signal and visibly separate from the default REAL workspace.');
