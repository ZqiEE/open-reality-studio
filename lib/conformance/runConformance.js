...
function main() {
  ...
  assert(realHardwarePanel.includes('const [expanded, setExpanded] = useState(false);') && realHardwarePanel.includes('const panelExpanded = primary || expanded'), 'REAL HARDWARE must be forced open only in REAL mode and default closed inside Simulation Lab.');
  ...
  assert(evidenceSidebar.includes("if (workspaceMode === 'real')") && evidenceSidebar.indexOf('{hardware}') < evidenceSidebar.indexOf('role="tablist"'), 'REAL HARDWARE must occupy a dedicated task rail outside the simulation evidence tabs.');
