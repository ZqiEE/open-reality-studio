...
assert(realDeviceWorkspace.includes('REAL DEVICE WORKSPACE') && realDeviceWorkspace.includes('workspaceDevices={[]}') && realDeviceWorkspace.includes('No virtual device is placed here'), 'The default REAL workspace must render no virtual device and must explain the honest disconnected state.');
...
assert(realHardwarePanel.includes('Reference-servo preflight (no signal)') && realHardwarePanel.includes('This is not the generic simulation workspace on the left.'), 'REAL command preview must be named as a hardware-local reference-servo preflight, never as a generic simulation mapping.');
