...
const supportGuideHtml = read('docs/SUPPORT.html');
...
const packageVerifier = read('scripts/verify-electron-package.cjs');
const page = read('app/page.tsx');
const realDeviceWorkspace = read('components/RealDeviceWorkspace.tsx');
...
const firmwareTargetAuthorization = read('lib/device-onboarding/FirmwareTargetAuthorization.ts');
const realExecutionReleaseApproval = read('lib/hardware/RealExecutionReleaseApproval.ts');
...
assert(hardwareIpc.includes('real_execution_locked'), 'UI real execution must stay locked until the version-bound release approval validates.');
assert(hardwareIpc.includes("'hardware', 'RealExecutionReleaseApproval'") && hardwareIpc.includes('verifyRealExecutionReleaseApproval') && !hardwareIpc.includes('readdirSync(EVIDENCE_DIR)'), 'Electron must consume the compiled release authority instead of counting arbitrary JSON files or copying policy.');
assert(hardwareIpc.includes("!app.isPackaged && process.env.ORS_REAL_EXECUTION === 'enabled'") && hardwareIpc.includes('no execution bypass is accepted in packaged mode'), 'The explicit source bench override must never unlock a packaged application.');
assert(realExecutionReleaseApproval.includes("appVersion: '0.5.1'") && realExecutionReleaseApproval.includes("profileId: 'esp32_s3_sg90_hc_sr04_v1'") && (realExecutionReleaseApproval.match(/sha256: '[a-f0-9]{64}'/g) || []).length === 4, 'REAL release approval must bind the app version, exact profile, and four reviewed evidence digests.');
assert(realExecutionReleaseApproval.includes('approved_evidence_digest_mismatch') && realExecutionReleaseApproval.includes('approved_evidence_semantics_mismatch') && realExecutionReleaseApproval.includes("physicalOutcomeVerified === false"), 'Release approval must fail closed on digest/semantic drift and preserve open-loop honesty.');
...
assert(hardwareIpc.includes("'device-onboarding', 'GovernedFirmwareImage'") && hardwareIpc.includes("'device-onboarding', 'FirmwareTargetAuthorization'") && !hardwareIpc.includes("from '../../lib/"), 'Electron must consume the compiled image and target authorities instead of importing or copying lib source.');
assert(firmwareTargetAuthorization.includes('target_port_not_listed') && firmwareTargetAuthorization.includes('firmware_target_changed') && firmwareTargetAuthorization.includes('firmware_authorization_expired'), 'First-flash target authority must bind a currently listed port, reject target swaps, and expire explicitly.');
assert(hardwareIpc.includes('consumeFirmwareAuthorization(payload.authorizationId)') && hardwareIpc.includes('delete(authorizationId); // one use') && hardwareIpc.includes('connection.listPortDescriptors()') && !hardwareIpc.includes('firmware_requires_connected_target'), 'Firmware preview must support blank boards while consuming a one-use authorization and rechecking the enumerated target.');
const flashFirmwareSource = hardwareIpc.slice(hardwareIpc.indexOf('async function flashGovernedFirmware'), hardwareIpc.indexOf('async function executeRealAngle'));
assert(flashFirmwareSource.indexOf('firmwareFlashInProgress = true;') < flashFirmwareSource.indexOf('await connection.listPortDescriptors()'), 'Firmware flashing must acquire its main-process concurrency lock before the first asynchronous target check.');
...
assert(hardwareIpc.includes('failed without retry') && hardwareIpc.includes("flashAttempted ? 'unknown_or_incomplete_after_failed_flash' : 'not_written'") && hardwareIpc.includes('diagnose did not verify firmware'), 'flashing must never retry silently, must distinguish pre-write failure from uncertain write state, and must verify the reported firmware version after reconnect.');
...
assert(page.includes("useState<'real' | 'simulation'>('real')") && page.includes("workspaceMode === 'simulation' && <LabConfigurator") && appHeader.includes('REAL DEVICE') && appHeader.includes('Dry run'), 'The desktop shell must default to a REAL-device workspace and expose simulation only as an explicit secondary mode.');
assert(realDeviceWorkspace.includes("telemetry.connected ? (") && realDeviceWorkspace.includes('data-real-twin-stage') && realDeviceWorkspace.includes('data-real-device-onboarding') && realDeviceWorkspace.includes('NO 3D STAGE · NO STALE DATA'), 'The disconnected REAL workspace must render a flat onboarding surface; the 3D twin stage may mount only after a current hardware connection.');
assert(realHardwarePanel.includes("setAvailable(bridge() !== null)") && realHardwarePanel.includes('data-real-hardware-bridge-ready'), 'REAL HARDWARE panel must re-detect the Electron bridge after hydration instead of remaining in SSR web mode.');
assert(page.includes("primary={workspaceMode === 'real'}") && realHardwarePanel.includes('const panelExpanded = primary || expanded') && realHardwarePanel.includes("primary ? 'min-h-0 flex-1'"), 'REAL mode must keep the hardware boundary expanded and give its task content the primary available height.');
assert(!evidenceSidebar.includes('data-real-hardware-primary="false"') && (evidenceSidebar.match(/{hardware}/g) || []).length === 1, 'Simulation Lab must not mount or display a REAL HARDWARE control strip.');
assert(appHeader.includes("'DISCONNECT → DRY RUN'") && appHeader.includes('Disconnect the REAL device and enter dry run (no hardware signal)'), 'Switching a connected REAL session into Simulation Lab must visibly disclose the disconnect before the mode change.');
assert(labConfigurator.includes('Simulation Lab has no hardware controls') && labConfigurator.includes('disconnects any REAL session and clears REAL telemetry'), 'Simulation Lab must explain that it owns no hardware controls and entering it clears the REAL session.');
assert(main.includes('!snapshot.realHardwareBoundary && !snapshot.realHardwarePrimary && !snapshot.realHardwarePanelReady') && main.includes("if (hardware) violations.push('real_hardware_visible_in_simulation')"), 'Electron smoke and design acceptance must require the REAL hardware surface to be absent in Simulation Lab.');
assert(main.includes("await setWorkspaceMode(window, 'real');") && main.includes("real_hardware_absent_in_simulation: 'passed'"), 'Forced-colors hardware inspection must return to REAL mode, while installed SIM journeys require no hardware boundary.');
assert(!realHardwarePanel.includes('AI Command Terminal runs simulation by default') && realHardwarePanel.includes('Simulation Lab is a separate zero-signal mode'), 'REAL HARDWARE copy must not describe simulation as the default product path.');
assert(realHardwarePanel.includes("type RealHardwareTool = 'command' | 'teach' | 'firmware'") && realHardwarePanel.includes('activeTool !== \'firmware\'') && realHardwarePanel.includes("activeTool === 'teach'"), 'Connected REAL tools must be separated into command, teach, and firmware tasks instead of one undifferentiated control stack.');
...
assert(realHardwarePanel.includes('Reference-servo preflight (no signal)') && realHardwarePanel.includes('it has no mapping to SIM LAB'), 'REAL command preview must be named as a hardware-local reference-servo preflight and deny any Simulation Lab mapping.');
...
assert(virtualDeviceStage.includes('SIMULATION · REAL MIRROR SEPARATE') && virtualDeviceStage.includes('SIMULATION · NO HARDWARE SIGNAL') && virtualDeviceStage.includes('READ-ONLY DIGITAL TWIN') && virtualDeviceStage.includes('border-status-warning-edge'), '1180px workspace must retain no-signal simulation semantics and a prominent black/yellow REAL mirror boundary.');
...
assert(realHardwarePanel.includes('release approved · REAL chain executable') && realHardwarePanel.includes('release approval invalid · default locked') && realHardwarePanel.includes('arbitrary JSON files and packaged environment variables cannot unlock it'), 'REAL UI must present release approval as a product state and give a truthful reinstall recovery path.');
...
assert(realHardwarePanel.includes('data-first-flash-entry') && realHardwarePanel.includes('No handshake required') && realHardwarePanel.includes("status !== 'connected' && status !== 'disconnected'"), 'A blank or unresponsive listed board must be able to prepare a governed first flash without claiming a firmware handshake.');
assert(realHardwarePanel.includes('firmwarePlan.authorizationId') && preload.includes('authorizationId: string') && preload.includes('expectedSha256, authorizationId, confirm'), 'Renderer-to-main flashing must return the exact ephemeral plan authorization instead of relying on a port path alone.');
...
assert(supportGuideHtml.includes('REAL-device first') && supportGuideHtml.includes('ESP32-S3 + SG90 + HC-SR04') && supportGuideHtml.includes('ECHO GPIO4') && supportGuideHtml.includes('5V→3.3V divider') && !supportGuideHtml.includes('simulation first'), 'Rendered offline support must lead with the exact reviewed REAL rig and its voltage-divider safety requirement.');
assert(supportGuideHtml.includes('SIM LAB is an explicit, secondary, zero-signal tool') && supportGuideHtml.includes('never replaces a failed REAL operation'), 'Rendered offline support must explain the narrow purpose of SIM LAB without promoting it to the product path.');
assert(supportActions.includes('REAL-device-first safety governance runtime.') && supportActions.includes('Current REAL scope: ESP32-S3 + SG90 + HC-SR04') && !supportActions.includes('Simulation-first safety governance desktop.'), 'About must state the REAL-first product truth instead of the legacy simulation-first positioning.');
...
assert(packageJson.build.extraResources.some((entry) => entry.from === 'docs/acceptance/evidence' && entry.to === 'support/acceptance/evidence' && entry.filter.length === 4), 'electron-builder must package exactly the four digest-bound REAL release evidence files.');
assert(packageVerifier.includes('verifyRealExecutionReleaseApproval') && packageVerifier.includes('packaged REAL execution approval failed'), 'Package verification must behaviorally validate the installed REAL release approval before accepting the installer.');
...
assert(evidenceSidebar.includes("'Run Decision'") && evidenceSidebar.includes("'Simulation Details'"), 'Evidence sidebar must name the two product-level outcomes in user language.');
assert(evidenceSidebar.includes("if (workspaceMode === 'real')") && evidenceSidebar.indexOf('{hardware}') < evidenceSidebar.indexOf('role="tablist"') && (evidenceSidebar.match(/{hardware}/g) || []).length === 1, 'REAL HARDWARE must exist only as the dedicated REAL task rail and never inside simulation evidence tabs.');
assert(evidenceSidebar.includes('data-real-task-sidebar') && evidenceSidebar.includes('data-real-hardware-primary="true"') && evidenceSidebar.includes('className="flex min-h-0 flex-1 flex-col'), 'REAL mode must give the governed hardware task boundary the full right rail instead of crowding it with duplicate status panels.');
assert(!page.includes("aria-label={language === 'zh' ? '真实设备状态'") && !page.includes("aria-label={language === 'zh' ? '真实设备安全契约'"), 'REAL status and safety copy must not be duplicated below the primary hardware task rail.');
assert(evidenceSidebar.includes('w-[320px]') && evidenceSidebar.includes('xl:w-[360px]') && main.includes('const expectedSidebarWidth = innerWidth < 1280 ? 320 : 360'), 'The right rail and Electron design acceptance must share the 1180/1440 responsive width contract.');
assert(evidenceSidebar.includes("useState<EvidenceTab>('evidence')") && evidenceSidebar.includes('onClick={() => setActiveTab(tab)}'), 'Evidence sidebar must default to run evidence while keeping simulation details explicitly selectable.');
assert(!evidenceSidebar.includes('previousSelection'), 'Workspace hydration or model selection must not replace the primary run-decision view without an explicit tab choice.');
...
assert(appHeader.includes("const realMode = props.workspaceMode === 'real'") && appHeader.includes('data-real-hardware-focus') && appHeader.includes('data-simulation-toolbar-action="marketplace"') && appHeader.includes('data-real-connection-state='), 'The toolbar must expose device controls and connection state in REAL mode while marking simulation-only actions for the explicit lab mode.');
assert(appHeader.includes("workspaceMode === 'simulation' ? [") && appHeader.includes('data-simulation-file-action=') && appHeader.includes('Import Device Manual (Simulation Only)'), 'The visible File menu must hide simulation imports in REAL mode and label them honestly inside Simulation Lab.');
assert(page.includes("if (action === 'run:virtualLab') {") && page.includes("setWorkspaceMode('simulation');") && page.includes("if (action === 'run:replay') {"), 'Native Virtual Lab run and replay actions must visibly enter Simulation Lab instead of running behind the REAL workspace.');
assert(page.includes("document.querySelector<HTMLElement>('[data-real-hardware-panel]')") && realHardwarePanel.includes('tabIndex={-1}') && realHardwarePanel.includes('focus:ring-status-warning'), 'The REAL device-control action must move focus visibly into the governed hardware boundary.');
assert(realDeviceWorkspace.includes("'点击自动检测'") && realDeviceWorkspace.includes('Auto-detect is read-only and never moves the servo.') && !realDeviceWorkspace.includes("zh ? '诊断设备' : 'Diagnose'"), 'Disconnected onboarding must describe the actual read-only auto-detect flow instead of inventing a separate Diagnose control.');
assert(realDeviceWorkspace.includes('a blank or unresponsive board enters governed first flash') && realHardwarePanel.includes('if (result.results[0]?.path) setSelectedPort(result.results[0].path);'), 'Auto-detect must hand an unresponsive but enumerated new board into the governed first-flash path instead of ending at a dead error state.');
assert(realDeviceWorkspace.includes('data-supported-real-rig') && realDeviceWorkspace.includes('ESP32-S3 + SG90 + HC-SR04') && realDeviceWorkspace.includes('5V→3.3V divider required') && realDeviceWorkspace.includes('outside the current executable product scope'), 'Disconnected REAL onboarding must define the exact reviewed rig and safety-critical ECHO wiring instead of assuming users know what reference rig means.');
...
for (const label of ['New Project', 'Open Project', 'Save Project', 'Export Simulation Lab Report', 'Export Simulation Adapter Package', 'Enter and Run Simulation Lab', 'Replay Simulation']) {
