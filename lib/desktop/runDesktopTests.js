...
assert(realHardwarePanel.includes('const [expanded, setExpanded] = useState(false);') && realHardwarePanel.includes('const panelExpanded = primary || expanded'), 'Simulation Lab must default the independent REAL HARDWARE boundary closed while REAL mode still forces it open.');
...
assert(evidenceSidebar.includes("if (workspaceMode === 'real')") && evidenceSidebar.indexOf('{hardware}') < evidenceSidebar.indexOf('role="tablist"'), 'REAL HARDWARE must be a dedicated task rail, structurally separate from simulation evidence tabs.');
