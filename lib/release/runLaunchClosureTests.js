...
const readiness = read('docs/RELEASE_READINESS_V0.5.1.md');
const desktopDocs = read('docs/DESKTOP_APP.md');
const nextConfig = read('next.config.mjs');

assert(nextConfig.includes("allowedDevOrigins: ['127.0.0.1']"), 'Electron development must allow the exact loopback host so HMR and local font resources stay current during product review.');

for (const selector of ['AppHeader', 'RealDeviceWorkspace', 'data-real-hardware-boundary']) {
  assert(main.includes(selector), `Packaged renderer smoke must verify ${selector}.`);
}
assert(main.includes('snapshot.realDeviceWorkspace && snapshot.commandDock && snapshot.realDeviceControl') && main.includes('snapshot.runControls === 1 && snapshot.stopControls === 1'), 'Packaged first-run smoke must verify the sole governed Run/Stop pair in the REAL-only workspace.');
assert(main.includes('snapshot.realHardwarePrimary') && main.includes('captureRealWorkspaceLayout') && main.includes('real_workspace_layouts') && main.includes('hardware.height < sidebar.height * 0.3'), 'Packaged and bilingual design acceptance must prove that REAL HARDWARE remains a substantial, focusable part of the evidence rail.');
assert(main.includes('snapshot.realDeviceControl') && main.includes('snapshot.simulationToolbarActions === 0') && main.includes('snapshot.simulationFileActions === 0') && main.includes('!snapshot.marketplaceTrigger') && main.includes('real_hardware_focus_failed'), 'Packaged REAL acceptance must reject simulation-only toolbar and File-menu leakage and prove the device-control focus path.');
assert(main.includes('disconnected_real_workspace_mimics_simulation') && main.includes('canvasCount !== 0') && main.includes('disconnectedOnboarding'), 'Packaged REAL acceptance must reject a disconnected 3D canvas or twin stage that visually masquerades as simulation.');
assert(!main.includes("setWorkspaceMode(window, 'simulation')") && main.includes('snapshot.simulationToolbarActions === 0') && main.includes('snapshot.simulationFileActions === 0'), 'Packaged smoke must keep the frozen Simulation Lab out of the primary product shell.');
...
assert(main.includes('snapshot.realHardwareBoundary') && main.includes('snapshot.realHardwarePrimary') && main.includes('snapshot.realHardwarePanelReady'), 'Packaged smoke must require the governed REAL boundary and device panel.');
...
assert(main.includes("hardwareBridge") && main.includes("typeof window.openReality?.hardware === 'object'") && main.includes('snapshot.realHardwarePanelReady'), 'Packaged first-run smoke must verify that the hardware preload bridge is visible to the hydrated REAL panel.');
assert(
...
assert(main.includes('snapshot.marketplaceBridge') && main.includes('!snapshot.marketplaceTrigger'), 'Packaged smoke must prove the frozen Marketplace has no primary-shell trigger.');
assert(!main.includes("auditDesignDialog(window, 'marketplace'"), 'Frozen Marketplace must not remain in the primary product-design dialog journey.');
...
assert(authenticode.includes("parsed.status !== 'Valid'") && authenticode.includes('timestamp is missing') && authenticode.includes('RLSOK.exe') && authenticode.includes('Setup.exe'), 'Authenticode verification must require Valid timestamped signatures for both EXE and installer.');
...
assert(lifecycleScript.includes('Refusing to alter an existing RLSOK installation'), 'Lifecycle verification must fail closed when a prior installation exists.');
...
try {
  ...
  fs.writeFileSync(path.join(fixtureRelease, 'RLSOK-9.8.7-Setup.exe'), 'installer-fixture');
  fs.writeFileSync(path.join(fixtureRelease, 'win-unpacked', 'RLSOK.exe'), 'executable-fixture');
  ...
  const fixtureLifecycle = {
    ...
    installer: { file: 'RLSOK-9.8.7-Setup.exe', sha256: crypto.createHash('sha256').update('installer-fixture').digest('hex').toUpperCase() },
  ...
  const fixtureDesign = {
    ...
    real_workspace_layouts: [
      { mode: 'real', viewport: { width: 1440, height: 900 }, language: 'zh', toolbar: { realDeviceControl: true, simulationToolbarActions: 0, simulationFileActions: 0, hardwareFocused: true }, workspaceState: { disconnectedOnboarding: true, realTwinStage: false, canvasCount: 0 }, violations: [] },
      { mode: 'real', viewport: { width: 1440, height: 900 }, language: 'en', toolbar: { realDeviceControl: true, simulationToolbarActions: 0, simulationFileActions: 0, hardwareFocused: true }, workspaceState: { disconnectedOnboarding: true, realTwinStage: false, canvasCount: 0 }, violations: [] },
      { mode: 'real', viewport: { width: 1180, height: 720 }, language: 'zh', toolbar: { realDeviceControl: true, simulationToolbarActions: 0, simulationFileActions: 0, hardwareFocused: true }, workspaceState: { disconnectedOnboarding: true, realTwinStage: false, canvasCount: 0 }, violations: [] },
      { mode: 'real', viewport: { width: 1180, height: 720 }, language: 'en', toolbar: { realDeviceControl: true, simulationToolbarActions: 0, simulationFileActions: 0, hardwareFocused: true }, workspaceState: { disconnectedOnboarding: true, realTwinStage: false, canvasCount: 0 }, violations: [] }
    ],
    layouts: [
      { viewport: { width: 1440, height: 900 }, language: 'zh', hardware: null, toolbar: { realDeviceControl: false, simulationToolbarActions: 5, simulationFileActions: 2 }, violations: [] },
      { viewport: { width: 1440, height: 900 }, language: 'en', hardware: null, toolbar: { realDeviceControl: false, simulationToolbarActions: 5, simulationFileActions: 2 }, violations: [] },
      { viewport: { width: 1180, height: 720 }, language: 'zh', hardware: null, toolbar: { realDeviceControl: false, simulationToolbarActions: 5, simulationFileActions: 2 }, violations: [] },
      { viewport: { width: 1180, height: 720 }, language: 'en', hardware: null, toolbar: { realDeviceControl: false, simulationToolbarActions: 5, simulationFileActions: 2 }, violations: [] }
    ],
  ...
  const designName = 'RLSOK-9.8.7-Design-Acceptance.json';
  ...
  const startupName = 'RLSOK-9.8.7-Startup-Acceptance.json';
  ...
  const signingName = 'RLSOK-9.8.7-Authenticode-Evidence.json';
  ...
  const signingEvidence = {
    ...
    artifacts: [
      { file: 'win-unpacked/RLSOK.exe', size_bytes: 18, sha256: executableHash, status: 'Valid', status_message: 'Signature verified.', signer: { subject: 'CN=Fixture Publisher', issuer: 'CN=Fixture CA', thumbprint: 'A'.repeat(40), not_before: '2026-01-01T00:00:00.000Z', not_after: '2027-01-01T00:00:00.000Z' }, timestamp: { subject: 'CN=Fixture TSA', thumbprint: 'B'.repeat(40) } },
      { file: 'RLSOK-9.8.7-Setup.exe', size_bytes: 17, sha256: expectedInstallerHash, status: 'Valid', status_message: 'Signature verified.', signer: { subject: 'CN=Fixture Publisher', issuer: 'CN=Fixture CA', thumbprint: 'A'.repeat(40), not_before: '2026-01-01T00:00:00.000Z', not_after: '2027-01-01T00:00:00.000Z' }, timestamp: { subject: 'CN=Fixture TSA', thumbprint: 'B'.repeat(40) } }
    ]
  };
...
const safeFixtureRoot = fs.mkdtempSync(path.join(os.tmpdir(), 'RLSOK-install-lifecycle-'));
try {
...
console.log('- Packaged smoke verifies the real renderer, preload bridge, sole governed Run/Stop pair, and REAL execution boundary.');
