...
assert(appHeader.includes('data-file-action={item.id}') && appHeader.includes("id: 'import-asset'") && appHeader.includes('data-simulation-file-action='), 'File actions must expose stable acceptance hooks and distinguish simulation-only imports.');
...
assert(appHeader.includes("role=\"group\"") && appHeader.includes("aria-pressed={props.workspaceMode === 'real'}") && appHeader.includes("aria-pressed={props.workspaceMode === 'simulation'}"), 'REAL and Simulation Lab modes must expose one keyboard-operable pressed-state group.');
assert(appHeader.includes('data-real-hardware-focus') && page.includes("panel?.focus()") && realHardwarePanel.includes('tabIndex={-1}') && realHardwarePanel.includes('focus:ring-status-warning'), 'REAL device-control navigation must land on a programmatically focusable boundary with visible focus treatment.');
...
assert(realHardwarePanel.includes("primary ? (") && realHardwarePanel.includes('<div className="flex h-8 w-full shrink-0 items-center') && realHardwarePanel.includes('aria-expanded={panelExpanded}'), 'The primary REAL boundary must use a non-interactive heading while the secondary boundary retains an honest expandable button.');
assert(realHardwarePanel.includes('aria-label={zh ? \'真实硬件任务\' : \'Real-hardware tasks\'}') && realHardwarePanel.includes('handleRovingTabKey(event, index, hardwareTools.length') && realHardwarePanel.includes('aria-controls={`real-hardware-panel-${tool}`}'), 'REAL command, teach, and firmware tasks must form one labelled keyboard-roving tab interface.');
