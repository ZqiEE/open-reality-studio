...
archive="$release_dir/rlsok-runtime-1.4.3-linux-x64.tar.gz"
...
test -f "$RLSOK_INSTALL_ROOT/1.4.3/lib/rlsok/experimental/ros2-reference-sidecar/rlsok_ros2_sidecar.py"
test -f "$RLSOK_INSTALL_ROOT/1.4.3/lib/rlsok/sdk/python/rlsok/__init__.py"
...
selected_hash=$(sha256sum "$RLSOK_INSTALL_ROOT/1.4.3/bin/rlsok" | cut -d' ' -f1)
printf 'previous-runtime-selected\n' > "$RLSOK_INSTALL_ROOT/1.4.3/acceptance-previous-runtime"
...
assert_selected_unchanged() {
  test "$(sha256sum "$RLSOK_INSTALL_ROOT/1.4.3/bin/rlsok" | cut -d' ' -f1)" = "$selected_hash"
  test "$("$RLSOK_BIN_DIR/rlsok" --version)" = "$candidate_version"
  test "$(cat "$RLSOK_INSTALL_ROOT/1.4.3/acceptance-previous-runtime")" = "previous-runtime-selected"
...
assert_injected_rollback() {
  ...
  assert_selected_unchanged
  test ! -e "$RLSOK_INSTALL_ROOT/1.4.3.new"
  test ! -e "$RLSOK_INSTALL_ROOT/1.4.3.rollback"
}
