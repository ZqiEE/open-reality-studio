...
function validateDesignEvidence(evidence) {
  ...
  for (const contract of [[1440, 900, 'zh'], [1440, 900, 'en'], [1180, 720, 'zh'], [1180, 720, 'en']]) {
    if (!layouts.some((item) => item?.viewport?.width === contract[0] && item?.viewport?.height === contract[1] && item?.language === contract[2] && item?.toolbar?.realDeviceControl === false && item?.toolbar?.simulationToolbarActions === 5 && item?.violations?.length === 0)) throw new Error(`Product-design viewport evidence missing: ${contract.join('x')}`);
    if (!realWorkspaceLayouts.some((item) => item?.mode === 'real' && item?.viewport?.width === contract[0] && item?.viewport?.height === contract[1] && item?.language === contract[2] && item?.toolbar?.realDeviceControl === true && item?.toolbar?.simulationToolbarActions === 0 && item?.toolbar?.hardwareFocused === true && item?.workspaceState?.disconnectedOnboarding === true && item?.workspaceState?.realTwinStage === false && item?.workspaceState?.canvasCount === 0 && item?.violations?.length === 0)) throw new Error(`REAL-workspace viewport evidence missing: ${contract.join('x')}`);
  }
