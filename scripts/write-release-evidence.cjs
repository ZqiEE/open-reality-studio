...
function validateDesignEvidence(evidence) {
  ...
  for (const contract of [[1440, 900, 'zh'], [1440, 900, 'en'], [1180, 720, 'zh'], [1180, 720, 'en']]) {
    ...
    if (!realWorkspaceLayouts.some((item) => item?.mode === 'real' && item?.viewport?.width === contract[0] && item?.viewport?.height === contract[1] && item?.language === contract[2] && item?.violations?.length === 0)) throw new Error(`REAL-workspace viewport evidence missing: ${contract.join('x')}`);
  }
